    ...
    <div class="row">
      <div class="col-md-6">
        <div class="form-group">
          ...
          <select v-model="product.laundry_type" class="form-control">
            <option value>Pilih</option>
            <option v-for="(row, index) in laundry_type" :key="index" :value="row.id">{{ row.name }}</option>
          </select>
      ...
      <div class="col-md-6" v-if="showForm">
        <div class="form-group" :class="{'has-error': errors.name_laundry_type}">
          ...
          <p class="text-danger" v-if="errors.name_laundry_type">{{ errors.name_laundry_type }}</p>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapState(["errors"]),
    ...mapState("product", {
      laundry_type: state => state.laundry_type
    })
  },
  methods: {
    ...mapActions("product", [
    ...
    addNewLaundryType() {
      this.addNewLaundryType({
        name_laundry_type: this.laundry_type
      }).then(() => {
        this.showForm = false;
        this.laundry_type = "";
      });
  ...
  submit() {
    ...
    } else if (this.$router.name == "products.edit") {
